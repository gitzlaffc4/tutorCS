...
$queryScheduled = "SELECT * FROM SCHEDULED_T INNER JOIN SESSION_T ON SCHEDULED_T.SESSION_ID = SESSION_T.SESSION_ID INNER JOIN COURSE_T ON SESSION_T.COURSE_ID = COURSE_T.COURSE_ID INNER JOIN TUTOR_T ON SESSION_T.TUTOR_HAWKID = TUTOR_T.HAWKID INNER JOIN USER_T ON TUTOR_T.HAWKID = USER_T.HAWKID WHERE SCHEDULED_T.COMPLETED = '0';";
